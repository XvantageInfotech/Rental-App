/**
 * Copyright 2025 Mujammil and other contributors
 * <p/>
...
class CustomOTPEditText : AppCompatEditText {
    protected var mMask: String? = null
    protected var mMaskChars: StringBuilder? = null
    protected var mSingleCharHint: String? = null
    protected var mAnimatedType: Int = 0
    protected var mSpace: Float = 24f //24 dp by default, space between the lines
    protected var mCharSize: Float = 0f
    protected var mNumChars: Float = 4f
    protected var mTextBottomPadding: Float = 8f //8dp by default, height of the text from our lines
    protected var mMaxLength: Int = 4
    ...
    protected var mIsDigitSquare: Boolean = false

    protected var mClickListener: OnClickListener? = null
    protected var mOnPinEnteredListener: OnPinEnteredListener? = null

    protected var mLineStroke: Float = 1f //1dp by default
    protected var mLineStrokeSelected: Float = 2f //2dp by default
    protected var mLinesPaint: Paint? = null
    protected var mAnimate: Boolean = false
    protected var mHasError: Boolean = false
    protected var mOriginalTextColors: ColorStateList? = null
    protected var mStates: Array<IntArray> = arrayOf(
    ...
    protected var mColorStates: ColorStateList = ColorStateList(mStates, mColors)
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val availableWidth = width - ViewCompat.getPaddingEnd(this) - ViewCompat.getPaddingStart(
            this
        )
    ...
    protected fun updateDrawableState(hasText: Boolean, isNext: Boolean) {
        if (mHasError) {
            ...
            if (isNext) {
                mPinBackground!!.setState(intArrayOf(android.R.attr.state_focused,android.R.attr.state_selected))
            } else if (hasText) {
                mPinBackground!!.setState(intArrayOf(android.R.attr.state_focused, android.R.attr.state_checked))
            }
        } else {
            if (hasText) {
                mPinBackground!!.setState(intArrayOf(android.R.attr.state_focused, android.R.attr.state_checked))
    ...
    private fun animatePopIn() {
        ...
        va.addUpdateListener { animation ->
            ...
            this@CustomOTPEditText.invalidate()
        }
